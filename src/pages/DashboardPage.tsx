      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
            ...
            <div className="text-sm text-slate-500 mt-1">Total Answered</div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
        ...
        {topicStats.length > 0 && (
          <div className="bg-white rounded-xl border border-slate-200 p-5 mb-8">
            <h2 className="font-semibold text-slate-800 mb-4">Performance by Topic</h2>
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {topicStats.sort((a, b) => a.percentage - b.percentage).map(ts => (
                <div key={ts.topicId}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-slate-700 truncate mr-2">{ts.name}</span>
                    <span className={`font-medium flex-shrink-0 ${
                      ts.percentage >= 70 ? 'text-green-600' : ts.percentage >= 50 ? 'text-amber-600' : 'text-red-600'
                    }`}>
        ...
        {sessions.length > 0 && (
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h2 className="font-semibold text-slate-800 mb-4">Session History</h2>
                ...
                return (
                  <div key={session.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                    <div>
                      ...
                      <div className="text-xs text-slate-400">{session.total} questions</div>
                    </div>
